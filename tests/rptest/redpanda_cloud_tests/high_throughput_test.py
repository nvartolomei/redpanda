...
class HighThroughputTest(PreallocNodesTest):
    ...
    def stage_tiered_storage_consuming(self):
        ...
        # Redpanda cloud tiers are asymetric when it comes to ingress and egress. Ingress is usually lower than egress.
        # Since we are interesting in introducing stress on the egress path, we need to adjust produce variables for that.
        target_consume_time_s = 10 * 60  # TODO(nv) increase when ready and scale it to trigger cloud upload
