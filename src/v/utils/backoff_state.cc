...
backoff_state::backoff_state(backoff_config cfg)
  : cfg_(cfg)
  , current_(cfg.initial) {}

seastar::future<std::optional<uint16_t>>
backoff_state::backoff(context_ref ctx) {
...
context::duration backoff_state::apply_jitter(context::duration d) const {
    if (cfg_.jitter <= 0.0) {
        return d;
    }
