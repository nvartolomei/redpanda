...
TEST(DatalakeMultiplexerTest, CorruptedBatchRecordHeader) {
    ...
    // Multiplex from start.
    {
        auto local_copy
          = batches | std::views::transform(&model::record_batch::copy)
            | std::ranges::to<chunked_circular_buffer<model::record_batch>>();

        auto reader = model::make_generating_record_batch_reader(
          [batches = std::move(local_copy)]() mutable {
              return ss::make_ready_future<model::record_batch_reader::data_t>(
                std::move(batches));
          });
