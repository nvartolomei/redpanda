...
private:
    tracing_span_data() noexcept = default;
    ...
    static tracing_span_data create_root(std::string_view root_name) noexcept {
...
public:
    ...
    OutputIt format_to(OutputIt out) const {
        ...
        while (current) {
            ...
            out = std::copy(current->name.begin(), current->name.end(), out);
...
///     parent, context::with<context::tracing_span>("operation_name")};
/// ```
class tracing_span {
    template<typename...>
...
private:
    ...
    void on_context_init(this Self& self, const char (&name)[N]) noexcept {
            ...
            ss::logger::lambda_log_writer writer(
              [&](ss::internal::log_buf::inserter_iterator it) {
                  ...
                  if (auto* span = ctx_.trace_span()) {
                      ...
                      switch (ctx_.cancel_cause()) {
                      case cancel_cause::manual:
                          // Mark manual cancellations with "!"
                          it = std::copy_n("!", 1, it);
                          break;
                          // NOLINTNEXTLINE(bugprone-branch-clone)
                      case cancel_cause::deadline:
                          // Cause obvious from deadline vs elapsed time.
                          break;
