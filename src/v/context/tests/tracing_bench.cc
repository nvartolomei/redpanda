...
PERF_TEST(tracing, traced_frame_create_1000) {
    ...
    for (int i = 0; i < 1000; ++i) {
        frame.emplace(
          context::background(),
          context::with<context::tracing_span>(context::span_name{"test_op"}));
        perf_tests::do_not_optimize(frame);
        frame.reset();
    }
    perf_tests::stop_measuring_time();
...
PERF_TEST(tracing, traced_context_tree_depth3_fanout20) {
    ...
    for (size_t i = 1; i < 3; ++i) {
        spine[i].emplace(
          ...
          context::with<context::tracing_span>(context::span_name{"spine"}));
    }
    // Build fanout from last spine node
    for (size_t i = 0; i < 20; ++i) {
        fanout[i].emplace(
          context_ref{*spine[2]},
          context::with<context::tracing_span>(context::span_name{"fanout"}));
    }
    // Build tails from each fanout
    for (size_t i = 0; i < 20; ++i) {
        tails[i][0].emplace(
          context_ref{*fanout[i]},
          context::with<context::tracing_span>(context::span_name{"tail"}));
        for (size_t j = 1; j < 2; ++j) {
            tails[i][j].emplace(
...
PERF_TEST(tracing, trace_logger_log_1000) {
    ...
    traced_frame frame{
      context::background(),
      context::with<context::tracing_span>(context::span_name{"operation"})};
