...
TEST(SpanTest, RootSpanCreation) {
    context::context_frame<context::tracing_span> root{
      context::background(),
      context::with<context::tracing_span>("root_operation")};

    auto* span = root.trace_span();
    ASSERT_NE(span, nullptr);
    EXPECT_EQ(span->name, "root_operation");
...
TEST(SpanTest, ChildSpanInheritance) {
    ...
    context::context_frame<context::tracing_span> child{
      root, context::with<context::tracing_span>("child")};
    ...
    EXPECT_EQ(child_span->name, "child");
}
...
TEST(SpanTest, InheritanceAcrossGaps) {
    ...
    context::context_frame<context::tracing_span> grandchild{
      middle, context::with<context::tracing_span>("grandchild")};
...
TEST(SpanTest, LoggingAcrossGaps) {
    ...
    EXPECT_EQ(found_span->name, "root");
}
...
TEST(SpanTest, TraceLoggerWithSpan) {
    context::context_frame<context::tracing_span> root{
      context::background(), context::with<context::tracing_span>("root_op")};
    context::context_frame<context::tracing_span> child{
      root, context::with<context::tracing_span>("child_op")};
    context::context_frame<context::tracing_span> grandchild{
      child, context::with<context::tracing_span>("grandchild_op")};
...
TEST(SpanTest, DumpTree) {
    context::context_frame<context::tracing_span> server{
      context::background(), context::with<context::tracing_span>("server")};

    context::context_frame<context::tracing_span> request_1{
      context::background(), context::with<context::tracing_span>("request_1")};
    auto request_1_link = context::link(request_1, server);

    context::context_frame<context::tracing_span> db_query{
      request_1, context::with<context::tracing_span>("db_query")};

    context::context_frame<context::tracing_span> request_2{
      context::background(), context::with<context::tracing_span>("request_2")};
...
TEST(SpanTest, SpanBacktrace) {
    ...
    using context::experimental::span_backtrace;

    context_ref gc_ref = grandchild;
    EXPECT_EQ(
      fmt::format("{}", span_backtrace(gc_ref)),
      "grandchild_op <- child_op <- root_op");
    EXPECT_FALSE(span_backtrace(gc_ref).empty());

    context_ref child_ref = child;
    EXPECT_EQ(
      fmt::format("{}", span_backtrace(child_ref)), "child_op <- root_op");

    context_ref root_ref = root;
    EXPECT_EQ(fmt::format("{}", span_backtrace(root_ref)), "root_op");

    context::context_frame<> no_span{context::background()};
    context_ref no_span_ref = no_span;
    EXPECT_TRUE(span_backtrace(no_span_ref).empty());
    EXPECT_EQ(fmt::format("{}", span_backtrace(no_span_ref)), "");
}
