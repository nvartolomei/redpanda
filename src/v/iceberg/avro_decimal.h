 ...
 *
 * The encoder produces a 1..16 byte payload sized for a `fixed[size]` slot,
 * with the value sign-extended on the left to fill it — matching the
 * Iceberg / Avro wire format. The decoder accepts 0..16 byte payloads,
 * covering both `fixed[N]` and the variable-width `bytes` form of
 * `decimal`, and sign-extends from the MSB of the first byte.
 */
...
inline bytes encode_avro_fixed_decimal(absl::int128 decimal, size_t size) {
    ...
    // Range check: a signed `size`-byte slot holds values in
