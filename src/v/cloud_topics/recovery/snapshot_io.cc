...
ntp_snapshot_id::parse(ntp_snapshot_prefix prefix, std::string_view str) {
    ...
    auto version_id_str = str.substr(
      str_prefix.size(),
      str.size() - str_prefix.size() - dot_snap_suffix.size());

    try {
        auto version = boost::lexical_cast<int64_t>(version_id_str);
        return ntp_snapshot_id(
          std::move(prefix), ntp_snapshot_version(version));
...
ss::future<std::optional<ntp_snapshot_id>> snapshot_io::find_latest_snapshot(
  ntp_snapshot_prefix prefix, retry_chain_node& rtc) {
    ...
    ntp_snapshot_id latest_id{prefix, ntp_snapshot_version{-1}};
    ...
    if (latest_id.version() == ntp_snapshot_version(-1)) {
