...
#include "cloud_topics/recovery/snapshot_version.h"
...
public:
    ntp_snapshot_id(
      ...
      model::partition_id partition_id,
      model::initial_revision_id rev_id,
      ntp_snapshot_version version)
      : _prefix(std::move(ns), std::move(tp), partition_id, rev_id)
      , _version(version){};

    ntp_snapshot_id(ntp_snapshot_prefix prefix, ntp_snapshot_version version)
...
public:
    ...
    const ntp_snapshot_version& version() const { return _version; }
