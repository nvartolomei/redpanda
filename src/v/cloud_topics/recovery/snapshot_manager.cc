...
ss::future<> snapshot_manager::stop() {
    vlog(lg.info, "Stopping snapshot manager");
    ...
    vlog(lg.info, "Snapshot manager stopped");
}

void snapshot_manager::attach_partition(
  ss::lw_shared_ptr<cluster::partition> p) {
    if (!is_cloud_partition(p)) {
        return;
    }
...
snapshot_manager::take_snapshot(ss::lw_shared_ptr<managed_partition> info) {
    ...
    // TODO(nv): Dummy.
    ntp_snapshot_version version{42};
    ...
    co_await _snapshot_io->upload_snapshot(id, iobuf{}, rtc);
